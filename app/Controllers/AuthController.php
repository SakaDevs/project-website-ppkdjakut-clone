...
class AuthController extends Controller
{
    ...
    public function facebookCallback()
    {
        ...
        try {
            // Ambil access token dari query string (code, state)
            $oauth->fetchAccessTokenWithAuthCode($this->request->getGet());

            // Ambil informasi user
            $userInfo = $oauth->fetchUserInfoWithToken();

            // Mapping data untuk Shield
            $mappedUser = $oauth->setColumnsName('login', $userInfo);

            // Contoh: login atau buat user baru
            // Tergantung ShieldOAuth kamu
            // Misalnya:
            service('auth')->attempt([
                'username' => $mappedUser['username'],
                'email'    => $mappedUser['email'],
                'password' => random_string('alnum', 16), // random password
            ]);
