...
<head>
    ...
    <title>Pelatihan MTU - PPKD Jakut</title>
    ...
    <div class="container mx-auto px-4 py-12 lg:py-16 mt-12">
        <div class="max-w-7xl mx-auto" data-aos="fade-up" data-aos-duration="1000">
            ...
            <div class="text-center mb-12">
                <h1 class="text-4xl font-bold text-gray-800 mb-4">List Pelatihan MTU</h1>
                <p class="text-gray-600 max-w-2xl mx-auto">Jelajahi berbagai program pelatihan Mobile Training Unit (MTU) yang kami tawarkan.</p>
            </div>

            <!-- Search Form -->
            <form class="max-w-lg mx-auto mb-10" method="get" action="<?= base_url('pelatihan/MTU/search') ?>">   
                ...
                <div class="relative">
                    ...
                    <input type="search" id="default-search" class="block w-full p-4 pl-12 text-sm text-gray-900 border border-gray-300 rounded-full bg-white focus:ring-cyan-500 focus:border-cyan-500" placeholder="Cari pelatihan MTU..." required name="keyword"/>
            ...
            <?php if(auth()->loggedIn() && auth()->user()->inGroup('admin')):?>
                <div class="text-center my-8">
                    <a href="<?= base_url('pelatihan/MTU/tambah') ?>" class="bg-green-600 hover:bg-green-700 text-white px-5 py-2.5 rounded-lg font-semibold shadow-sm transition-transform hover:scale-105 inline-block">
                        + Tambah Pelatihan MTU
                    </a>
                </div>
            <?php endif; ?> 
                    ...
                    <?php foreach ($pelatihan as $row): ?>
                        <div class="pelatihan-card bg-white rounded-xl shadow-md overflow-hidden border border-gray-200">
                            <img class="w-full h-48 object-cover" 
                                 src="<?= base_url('uploads/foto_pelatihan_MTU/' . $row['gambar']) ?>" 
                            ...
                            <div class="p-5 flex flex-col flex-grow">
                                ...
                                <div class="mt-auto">
                                    <a href="/pelatihan/MTU/daftar" class="block w-full text-center bg-cyan-500 hover:bg-cyan-600 text-white font-bold py-2 px-4 rounded-lg transition duration-300">
                                        Daftar Sekarang
                                    </a>
                                    
                                    <?php if(auth()->loggedIn() && auth()->user()->inGroup('admin')):?>
                                        <div class="flex items-center justify-center gap-x-6 mt-4">
                                            <a href="/pelatihan/MTU/edit/<?= $row['id']?>" class="font-medium text-green-600 hover:underline text-sm">
                                                Edit
                                            </a>
                                            <form action="/pelatihan/MTU/delete/<?= $row['id'] ?>" method="post" class="m-0" onsubmit="return confirm('Apakah kamu yakin ingin menghapus pelatihan ini?');">
                                                <?= csrf_field() ?>
